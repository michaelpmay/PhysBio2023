...
\title{Exploiting Intrinsic Noise to Control Cell Differentiation Dynamics in the Presence of Time Delays, Inexact Models, and Extrinsic Variability}
\maketitle
...
In this case, Brownian motion, discrete stochastic gene regulation, and mRNA dynamics collectively generate a fundamentally noisy environment that cells must effectively manage. 
...
$\kappa$         & 0.406   & Molecules Per Minute \\ \hline
$\beta$          & 20.0    & Molecules            \\  \hline
$\eta $           & 8.00    & unit less          \\  \hline
$k_0$           & 0.0001  & Molecules per Minute \\  \hline
$\gamma$         & 0.0203  & per Minute         \\  \hline
...
These definitions of the stoichiometry and propensity functions allow us to implement the Gillespie stochastic simulation algorithm\cite{Gillespie1992, Gillespie1977} to generate representative trajectories of the stochastic process. 

\brian[Add a short paragraph explaining the basic SSA. This should include explicit statement of how the two RVs are generated: the time of the next reaction and the choice of which reaction occurs. This is needed since you will modify it below for the delayed reaction SSA.]
...
\brian[Add a paragraph introducing the FSP truncation and include some justification.]

\subsection{Quantification and optimization of control performance}\label{sec:Quantification}
Using optogenetic control, it is assumed that only a single light input would be applied at a given time and that it would be applied uniformly to multiple cells at the same intensity. 
In this single-input-multiple-output (SIMO) control process, any input signal applied to one cell affects all cells in the population. 
Thus, it is not possible to independently control individual cells without impacting others, so an effective controller must strike a balance among the desired behaviors of all cells in the system.
...
To quantify performance success, we define the {\em steady state performance error}, $J$, as the expected steady state Euclidean distance of the process from the specified target state, $\mathbf{T}$:
 ...
 \end{equation}
This score is easily calculated by applying a linear operation on steady state probability distribution $\mathbf{P}^*$ (Eq.\ \ref{SSDist}) as follows:
\begin{align}
...
In \cite{May2021}, we optimized each of these two controllers to find the control laws $\mathbf{u}^{FAC}$ and $\mathbf{u}^{PAC}$ that minimize $J$ for a single target state $\mathbf{T} = [XXX,XXX]$. 
These controllers are assumed to be fixed for the current study unless noted otherwise. 
...
When running the stochastic simulation algorithm under this fluctuating control signal, a slight modification is needed to the SSA.
...
Because optimizing the control law for a static set point as described in Section \ref{sec:Quantification} requires differentiation of the CME (Eq. \ref{XXX}) with respect to the control signal at each state, this calculation is approaching the limits of currently feasibility.
...
To circumvent this challenge, we instead propose a simple alternative to sweep over a set of many different steady state controllers in piecewise fashion to mimic a dynamically moving path.

\brian[I cannot understand the following section -- please rewrite.]
...
\brian[Add a paragraph or two to explain the big picture here.  Use figure 1 to remind the reader of (1) the control goals, (2) the control law, (3) the resulting distributions, (4) the objective function and (5) the optimization of the control law to minimize the error.  Refer to the methods for each such calculation.  This section is critical to make sure the reader understands what is coming next.  Then provide an outline of the remaining subsections of the results section so that the reader knows what to expect.]
...
To asses the control performance to be expected under such parameter errors, we performed sensitivity analysis on each model parameter for each cell, then on both cells simultaneously. 
...
In each case the baseline parameters is denoted as a vertical dashed line, and lower values of the RMSE ($J$ in Eq. \ref{XXX}) denote improved performance. 
...
\brian[The remainder of this section is confusing and seems out of place.  Please revise to remove redundancies.]

In most cases, changing parameters had similar effects on control performance for both $\mathbf{u}^{\rm FAC}$ (solid lines) and $\mathbf{u}^{\rm PAC}$ (dashed lines). However, for some parameters like $\kappa$ or $\gamma$, the PAC controller showed greater robustness to parameter errors than the FAC controller, and the PAC outperforms the FAC under several parameter perturbation conditions (compare dashed and solid lines). We note that this effect is due entirely to the mismatch between the assumed and actual parameters - if one were to recalibrate the model and re-optimizing the controllers for the new parameters, the FAC controller would always yield better control performance compared to the PAC.
