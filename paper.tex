...
These definitions of the stoichiometry and propensity functions allow us to implement the Gillespie stochastic simulation algorithm (SSA) \cite{Gillespie1992, Gillespie1977} to generate representative trajectories of the stochastic process.This algorithm uses discrete reactions and discrete molecule counts to sample the chemical master equation by utilizing an exponential random variable for time intervals and a categorical random variable for reaction selection. 
The exponential random variable is given by
\begin{equation}
\delta t(\mathbf{X},t)=log(rand \sum w_i(\mathbf{X},t) )
\end{equation}
where $\delta t$ is the time at which the next reaction occurs, and rand is a randomly generated number between 0 and 1.
The probability of an individual reaction to fire from all possible reactions is given by
\begin{equation}
\zeta_i(\mathbf{X},t)=\frac{w_i(\mathbf{X},t)}{\sum w_i(\mathbf{X},t)}
\end{equation}
Where $\zeta_i(\mathbf{X},t)$ is the probability of the ith reaction to occur at state $\mathbf{X}$ and time $t$. Then the reaction that fired at time $t$ is a categorical random variable pulled from $\zeta_i(\mathbf{X},t)$. Then the SSA can be created by walking through time jumps and updating the state of the system by adding the stiochiometry of the reaction to the state of the system.
...
To quantify performance success, we define the {\em steady state performance error}, $J$, as the expected steady state Euclidean distance of the process from the specified target state, $\mathbf{T}$:
 ...
 \end{equation}
This score is calculated by applying a linear operator on steady state probability distribution $\mathbf{P}^*$ (Eq.\ \ref{SSDist}) as follows:
\begin{align}
...
Optimization of $\mathbf{u}^{FAC}$ was performed using a gradient descent method to minimize $J$. This method requires calculating the negative gradient $-\frac{dJ}{d\mathbf{u}^{FAC}}$ and walking in a small step $du$ in that direction. The calculation of the gradient is given by
\begin{equation}
\frac{dJ}{d\mathbf{u}^{FAC}}=C \frac{dP}{d\mathbf{u}^{FAC}}
\end{equation}
...
Optimization of $\mathbf{u}^{PAC}$ was done by restricting $\frac{dJ}{d\mathbf{u}^{FAC}}$ in ambient space to search a low dimensional subspace with a linear lift operator D. Then the subspace search is given by. 
\begin{equation}
\frac{dJ}{d\mathbf{u}^{PAC}}=C \frac{dP}{d\mathbf{u}^{PAC}} = C D \frac{dP}{d\mathbf{u}^{FAC}}  
\end{equation}
Root mean squared error was calculated by taking the square root of the score.
\begin{equation}
RMSE=\sqrt{J}
\end{equation}
Since the square root is a monotonically increasing function, minimizing J is the same as minimizing RMSE. 

In \cite{May2021}, we optimized each of these two controllers to find the control laws $\mathbf{u}^{FAC}$ and $\mathbf{u}^{PAC}$ that minimize $J$ for a single target state $\mathbf{T} = [10,30]$, but in this work extend it to ant target point in the state space.
...
To circumvent this challenge, we instead propose a simple alternative to sweep though a set of controllers over time which target different static points in piecewise fashion to mimic a dynamically moving path.
 ...
 We start by describing a path $\bf{p}_n(x)$ in state space which walks over a two dimensional path in a discrete number of steps. Then, optimize a controller $\bf{c}_n(x)$for each target point and cycling through the controllers over time to create $\mathbf{r}(x,t)$.
