...
\brian[Let's remove the PAC from this section.  Show only the FAC and and replace the Fig 6C/D with the FAC at a different target point. Put a new star of a different color to match to the new 6C/D.  Remove Fig 6F.  Make sure to change all the figure labels and titles and captions.]

Figure \ref{CtrlP}(E) shows the FAC control performance over the entire domain of static set points, and illustrates that some regions are easier to obtain than others. For example, near the two stable fixed points at [XXX,XXX] and [XXX,XXX], the control performance is quite strong (white regions), whereas near the unstable point [20,20], the FAC results in relatively worse control. 

Edge regions were difficult to control, likely due to the geometry of how control inputs move the probability. \brian[I don't understand this statement.]
...
To examine how driving frequency and system scale affects control performance, 64 or 128 SSA trajectories of the phase-separated dynamic controller were simulated over a two dimensional domain of points $(\alpha,f)$ for 16 or 32 cycles after reaching steady state. Figure \ref{CRG}(A) shows the desired reference signal for the system, and Fig \ref{CRG}(B) shows the mean of the system response when the frequency is $f=10^{-4}$ \brian[Need units] and the granularity is $\alpha = 4.0$.  Figure \ref{CRG}(C) shows the corresponding control performance over normalized periodic time as $\alpha$ is held at $4.0$ and $f$ is increased from $f=10^{-4}$ \brian[Units] to $f=10^{-2}$ \brian[Units] to $f=10^{-0}$ \brian[Units] leading to average control performances of 6.2, 16.1, and 15.0 respectively. 
...
In particular, the heat map of control performance over $(f,\alpha)$ shows that the worst performance occurs at moderate $f$ near $f=10^{-2}$ and that control performance is best when granularity is high and frequency is low.
