...
Motivated by optogentics, we previously proposed a difficult control problem which requires the exploitation of noise in a stochastic system to break symmetry between two signals. We found multiple such controllers which could exploit noise to break symmetry between two cells under a variety of system information.
...
Using discrete stochastic models based on the chemical master equation, it was demonstrated that combining biochemical noise, nonlinear auto-regulation, and a single optogenetic feedback could control two genetically identical cells with different initial conditions to reach different desired signals at different frequency and phase (Figure \ref{cartoons}B). Figure\ \ref{cartoons}C(left) presents heat maps for two control laws, which a single external UV excitation signal based on observed cellular state.  Figure\ \ref{cartoons}C(right) shows the corresponding achievable marginal stationary distributions. Effective control was possible with both Fully Aware Feedback Controllers (FAC, Figure\ \ref{cartoons}C(top)) and Partially Aware Feedback Controllers (PAC, Figure\ \ref{cartoons}C(bottom)), even when the control model was an approximate simplification of the actual process dynamics.

Using discrete stochastic models based on the chemical master equation, this study demonstrates simultaneous control of two genetically identical cells with different initial conditions, by exploiting noise, nonlinear auto-regulation, and using a single input.  Heat maps in Figure \ref{cartoons}C(left) illustrate two control laws based on observed cellular activity, while Figure\ \ref{cartoons}C(right) shows the corresponding achievable marginal stationary distributions. Effective control was achieved using both Fully Aware Feedback Controllers (FAC, Figure\ \ref{cartoons}C(top)) and Partially Aware Feedback Controllers (PAC, Figure\ \ref{cartoons}C(bottom)). 
...
static setpoint control is the control of a reference point which does not move over time. 
dynamic setpont control targets a mvonig referenc peoint.
synamis serpoint control is hard to deriuve, but the optimization of peicesice constangtof each static referenc pont is easy and can be stiched together. 
...
systems driven at high frequency often have thier control performance reduced due to the response time scale of the system they are driving, but in a stochastic system the combination of high frequency dirving and noise could be catastriphic. 
...
Sixty-four SSA trajectories of the phase-separated dynamic controller were simulated over a two dimensional domain of points $(\alpha,f)$ for 16 cycles after reaching steady state. Increasing $\alpha$ while holding $f$ constant at $XXX$ showed improved control performance at low frequency but little change at high frequency Fig \ref{CRG}(D). Analysis of control performance using varying frequency but constant $\alpha$ showed the worst performance at moderate $f$. 
