...
Uncertain fluctuations, or `noise,' is a common theme throughout many fields of engineering, and is a frequent concern when attempting to control any human-made system such as vehicles \brian[\cite{XXX}], production machinery \brian[\cite{XXX}], or chemical processes \brian[\cite{XXX}].
...
For many macroscopic, human-engineered systems, the resulting system dynamics can be modeled effectively simply by combining a deterministic (i.e., noise-free) model with additive noise (usually assumed to be Gaussian under arguments based on the central limit theorem) \brian[\cite{XXX}], and most current approaches seek to control the system to minimize variations that result from the noisy inputs. 

Analyses of noise and control theory are equally relevant to understand the basic biological processes of gene transcription regulation \brian[\cite{XXX}] or mRNA translation regulation \brian[\cite{XXX}]or to modify these processes for practical use \brian[\cite{XXX}].
...
Classical and modern control methods like PID control and model predictive control have been implemented in such systems \brian[\cite{XXX}] to control synthetic systems to different stable points. 
...
In May et al.~\cite{May2021}, we identified a simplified stochastic model to reproduce data measured in Baumschlager, et al. \brian[\cite{XXX}] for the expression from a transcription promoter under the optogenetic control of a UV-activated T7 polymerase (see model in Figure \ref{cartoons}A, top promoter).
...
Both models were independently parameterized using the same experimental data from Baumschlager et al. \brian[\cite{XXX}]. 
...
For example, when analyzing discrete stochastic chemical kinetics, it is common to project the CME onto lower-dimensional spaces using finite state projection \brian[\cite{XXX}], time scale separations \brian[\cite{Peles2006}], Krylov subspaces \brian[\cite{Macnemara&Sidje?}], principle orthogonal decompositions \brian[\cite{Vo2019}], or other coarse meshes \brian[\cite{Munsky2008IEEE,Tapia2012CDC}]. 
...
\brian[Label 'J' is missing from the figure. Replace XXX below.]
